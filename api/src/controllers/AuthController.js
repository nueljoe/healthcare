...
export default {
  ...
  async createAccount(req, res, next) {
    ...
    try {
      const userWithSameEmail = await knex
        .first("id")
        .from("users")
      ...
      const [userId] = await knex("users").transacting(transaction).insert({
        email: body.email,
      ...
      await Promise.all([
        knex("user_profiles").transacting(transaction).insert(
          {
        ...
        knex("subscribers").transacting(transaction).insert({
          email: body.email,
        }),
      ]);
      ...
      await mailer.sendAccountVerification({
        ...
        last_name: body.last_name,
        email: body.email,
        verification_url: `${env.get(
          "FRONTEND_BASEURL"
        )}/auth/verify?token=${confirmationToken}`,
      });
  ...
  async changePassword(req, res, next) {
    ...
    try {
      const currentPasswordIsValid = await _bcrypt.compare(
    ...
    try {
      ...
      if (!user) {
        throw new NotFoundError(
          "We are unable to identify your account. Please check your email"
        );
      }

      if (user.verified_at) {
        throw new PermissionError(
          "Cannot perform this operaion on a verified account"
        );
      }
  ...
  async resetPassword(req, res, next) {
    ...
    try {
      ...
      const passwordTokenObject = await knex
